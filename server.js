...
function validateTikTokCallback(req, res, next) {
  ...
  if (error) {
    return res
      .status(400)
      .send(`<h1>Login Failed</h1><p>${error}: ${error_description || ""}</p>`);
  }

  if (!code || !state || !savedVerifier) {
    return res
      .status(400)
      .send("<h1>Invalid Request</h1><p>Missing or invalid state/code.</p>");
  }
  ...
  try {
    ...
    if (user.display_name) {
      const tiktokLive = new WebcastPushConnection(user.display_name, {
        eulerApiKey: process.env.EULER_API_KEY, // set in your .env file
      });
      ...
      tiktokLive
        ...
        .then((state) => {
          console.log(
            `🎥 Connected to LIVE room ${state.roomId} for @${user.display_name}`
          );
    ...
    return res
      .status(500)
      .send("<h1>Server Error</h1><p>Failed to exchange token.</p>");
  }
}
